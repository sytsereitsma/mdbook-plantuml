...
mod tests {
    ...
    fn test_save_downloaded_image() {
        ...
        let mut img_path = srv
            .save_downloaded_image(&data, &String::from("ext"))
            .unwrap();
        assert!(img_path.starts_with("img/"));
        img_path.drain(..4); //Strip "img/" part

        let fs_image_path =
            PathBuf::from(format!("{}/{}", output_path.to_str().unwrap(), img_path));
    ...
    fn test_render_string() {
        ...
        let mut img_path = srv.render_string(&String::from("C --|> D"), &mock_downloader).unwrap();
        assert!(img_path.starts_with("img/"));
        img_path.drain(..4); //Strip "img/" part
